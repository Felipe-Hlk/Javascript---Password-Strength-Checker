const passwordLength = () => {
    ...
    if (inputVal.length < 5) {
        inputLen.innerHTML = 'Minimum password length is 5';
        document.querySelector('.progress').style.display = 'none'
        return;
    }
    ...
    if(inputVal.length > 25) {
        inputLen.innerHTML = 'Maximum password length is 25';
        document.querySelector('.progress').style.display = 'none'
        return
    }
...
const clickToCheck = (event) => {
    ...
    if(!inputVal.includes(' ') && inputVal.length > 5 && inputVal.length <= 25) {
